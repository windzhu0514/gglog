...
func (s *severity) Set(value string) error {
	...
	logging.logthreshold.set(threshold)
	return nil
}
...
// LogThresholdHandler is a simple JSON endpoint that can report on or change the current
// logging severity.
//
// GET requests return a JSON description of the current logging severity. PUT
// requests change the logging level and expect a payload like:
//   {"level":"info"}
//
// It's perfectly safe to change the logging severity while a program is running.
func LogThresholdHandler() http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		type errorResponse struct {
			Error string `json:"error"`
		}
		type payload struct {
			Logthreshold string `json:"logthreshold"`
		}
		...
		case http.MethodGet:
			enc.Encode(payload{Logthreshold: logging.logthreshold.String()})
		...
		case http.MethodPut:
			...
			if errmess := func() string {
				...
				// Is it a known name?
				if v, ok := severityByName(req.Logthreshold); ok {
					threshold = v
				} else {
					v, err := strconv.Atoi(req.Logthreshold)
					if err != nil {
						return err.Error()
					}
					threshold = severity(v)
				...
				if threshold < 0 || threshold >= numSeverity {
					return "Must specify a valid severityName or severity value."
				}
				return ""
			}(); errmess != "" {
...
// OptToStderr set log to standard error instead of files
// if set this option,log_dir not work
func OptToStderr() option {
	return optionFunc(func() {
		logging.toStderr = true
	})
}

// OptAlsoToStderr set log to standard error as well as files
func OptAlsoToStderr() option {
	return optionFunc(func() {
		logging.alsoToStderr = true
	})
}

// OptLogDir set directory in which to write logs
func OptLogDir(dir string) option {
	return optionFunc(func() {
		logging.logDir = dir
	})
}

// OptVerbosity set log level for V logs
func OptVerbosity(v int) option {
	return optionFunc(func() {
		logging.verbosity = Level(v)
	})
}

// OptLogThreshold set log threshold,omit logs at or below this threshold
func OptLogThreshold(value string) option {
	return optionFunc(func() {
		var threshold severity
		// Is it a known name?
		if v, ok := severityByName(value); ok {
...
type loggingT struct {
	...
	logthreshold severity // omit logs at or below this threshold
...
func (sb *syncBuffer) rotateFile(now time.Time) error {
	...
	sb.file, isFileExist, err = create(now)
	sb.nbytes = 0
	if err != nil {
		return err
	}
	...
	if !isFileExist {
		...
		fmt.Fprintf(&buf, "Log line format: yymmdd hh:mm:ss.uuuuuu [IWEF] file:line] msg\n")
		var n int
